...
export default function AdminPanel() {
  ...
  // Variants panel (shown after a saree is saved/opened)
  const [variantSaree, setVariantSaree]   = useState(null); // full saree object
  ...
  useEffect(() => {
    if (token) { loadSarees(); loadOrders(); }
  }, [token, loadSarees, loadOrders]);
  ...
  // ── Reload a single saree (for variant panel refresh) ─────
  async function reloadSaree(id) {
    try {
      const fresh = await apiFetch('GET', `/api/sarees/${id}`, null, token);
      setVariantSaree(fresh);
      // Also update it in the sarees list
      setSarees(prev => prev.map(s => s._id === id ? { ...s, variants: fresh.variants } : s));
    } catch (e) { notify(e.message, false); }
  }
  ...
  async function openVariants(s) {
    // Fetch full saree (includes variants array)
    try {
      const full = await apiFetch('GET', `/api/sarees/${s._id}`, null, token);
      setVariantSaree(full);
      setVariantForm({ color:'', price:'', stock:'', sku:'' });
      setVariantImgFiles({});
  ...
  async function addVariant(e) {
    ...
    if (!variantForm.color || !variantForm.price || variantForm.stock === '') {
      notify('Colour, price and stock are required', false); return;
    }
    ...
    try {
      ...
      await apiFetch('POST', `/api/sarees/${variantSaree._id}/variants`, payload, token);
      notify(`Colour "${variantForm.color}" added ✓`);
      setVariantForm({ color:'', price:'', stock:'', sku:'' });
      await reloadSaree(variantSaree._id);
    } catch (err) { notify(err.message, false); }
    finally { setVariantSaving(false); }
  }

  // ── Upload images for a variant ───────────────────────────
  async function uploadVariantImages(variantId) {
    const files = variantImgFiles[variantId];
    if (!files?.length) { notify('Select images first', false); return; }
    try {
      const fd = new FormData();
      files.forEach(f => fd.append('images', f));
      await apiFetch('POST', `/api/sarees/${variantSaree._id}/variants/${variantId}/images`, fd, token, true);
      notify(`${files.length} image(s) uploaded ✓`);
      setVariantImgFiles(prev => ({ ...prev, [variantId]: [] }));
      await reloadSaree(variantSaree._id);
    } catch (e) { notify(e.message, false); }
  }
  ...
  async function updateVariantStock(variantId, currentStock) {
    const val = window.prompt('New stock:', currentStock);
    if (val === null || val === '' || isNaN(val)) return;
    try {
      await apiFetch('PATCH', `/api/sarees/${variantSaree._id}/variants/${variantId}/stock`, { stock: Number(val) }, token);
      notify(`Stock → ${val}`);
      await reloadSaree(variantSaree._id);
    } catch (e) { notify(e.message, false); }
  }
  ...
  // ── Delete a variant ──────────────────────────────────────
  async function deleteVariant(variantId, color) {
    if (!window.confirm(`Delete "${color}" variant? Images on S3 are not removed.`)) return;
    try {
      await apiFetch('DELETE', `/api/sarees/${variantSaree._id}/variants/${variantId}`, null, token);
      notify(`"${color}" variant deleted`);
      await reloadSaree(variantSaree._id);
    } catch (e) { notify(e.message, false); }
  }
  ...
  const vset = (k, v) => setVariantForm(p => ({ ...p, [k]: v }));
  const f = form;
  ...
  return (
    <div style={A.wrap}>
      ...
      <div style={A.main}>
        ...
        <div style={A.topbar}>
          <div>
            <div style={A.pageTitle}>{tab==='sarees' ? 'Saree Inventory' : 'Orders'}</div>
            <div style={A.pageSub}>
              {tab==='sarees'
                ? `${sarees.length} total · ${sarees.filter(s=>s.isActive).length} active`
                : `${orders.length} orders · ${orders.filter(o=>o.status==='pending').length} pending`}
            </div>
          </div>
          {tab==='sarees' && (
            <div style={{ display:'flex', gap:12, alignItems:'center' }}>
              <input style={A.searchInput} placeholder="Search sarees…"
                value={search} onChange={e => setSearch(e.target.value)} />
              <button style={A.addBtn} onClick={openNew}>+ Add Saree</button>
            </div>
          )}
        </div>
        ...
        {tab==='sarees' && (
          <div style={A.content}>
            {loading ? (
              <div style={A.loadingWrap}><div style={A.spinner}/><span>Loading…</span></div>
            ) : (
              <div style={A.tableCard}>
                <table style={A.table}>
                  <thead>
                    <tr>
                      {['Image','SKU','Name','Type','Price','Stock','Variants','Featured','Status','Actions'].map(h => (
                  ...
                  <tbody>
                    {filteredSarees.map((s, i) => (
                      <tr key={s._id} style={{ ...A.tr, opacity:s.isActive?1:0.45, background:i%2===0?'transparent':'rgba(255,255,255,0.015)' }}>
                        <td style={A.td}>
                          {s.imageUrls?.[0]
                            ? <img src={s.imageUrls[0]} alt="" style={A.thumb}/>
                            : <div style={A.thumbPlaceholder}>🥻</div>}
                        </td>
                        <td style={A.td}><span style={A.skuBadge}>{s.sku}</span></td>
                        <td style={A.td}>
                          <div style={{ fontWeight:500, color:'#F5F0E8', fontSize:13 }}>{s.name}</div>
                          <div style={{ fontSize:11, color:'#5A4A30', marginTop:2 }}>{s.color}</div>
                        </td>
                        <td style={A.td}><span style={A.typeBadge}>{s.type}</span></td>
                        <td style={A.td}>
                          <div style={{ color:'#D4AB5A', fontWeight:600 }}>₹{s.price?.toLocaleString('en-IN')}</div>
                          {s.originalPrice && <div style={{ color:'#5A4A30', fontSize:11, textDecoration:'line-through' }}>₹{s.originalPrice?.toLocaleString('en-IN')}</div>}
                        </td>
                        <td style={A.td}>
                          <span style={s.stock===0?A.stockRed:s.stock<5?A.stockAmber:A.stockGreen}>{s.stock}</span>
                          {' '}
                          <button style={A.btnTiny} onClick={()=>quickStock(s)}>Edit</button>
                        </td>

                        {/* Variants count + manage button */}
                        <td style={A.td}>
                          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                            <span style={{ color:'#8A7560', fontSize:13 }}>
                              {s.variants?.length || 0} colour{s.variants?.length !== 1 ? 's' : ''}
                            </span>
                            <button
                              style={{ ...A.btnTiny, color:'#D4AB5A', borderColor:'#5A4010' }}
                              onClick={() => openVariants(s)}
                            >
                              Manage
                            </button>
                          </div>
                        ...
                        <td style={A.td}>
                          <button style={s.isFeatured?A.pillOn:A.pillOff} onClick={()=>toggleField(s,'isFeatured')}>
                            {s.isFeatured?'★ Yes':'☆ No'}
                          </button>
                        </td>
                        <td style={A.td}>
                          <button style={s.isActive?A.pillGreen:A.pillGray} onClick={()=>toggleField(s,'isActive')}>
        ...
        {tab==='orders' && (
          <div style={A.content}>
            <div style={A.tableCard}>
                  ...
                  {orders.map((o,i) => {
                    ...
                    return (
                      <tr key={o._id} style={{ ...A.tr, background:i%2===0?'transparent':'rgba(255,255,255,0.015)' }}>
                        ...
                        <td style={A.td}><span style={{ color:'#D4AB5A', fontWeight:600 }}>₹{o.pricing?.grandTotal?.toLocaleString('en-IN')}</span></td>
                        <td style={A.td}>
                          <select
                            style={{ ...A.statusSelect, background:sc.bg, color:sc.color, borderColor:sc.border }}
                            value={o.status}
              ...
              {orders.length===0 && <div style={{ padding:'3rem', textAlign:'center', color:'#6B5E4A' }}>No orders yet</div>}
            </div>
          </div>
        )}
      </div>
            ...
            <form onSubmit={saveSaree} style={A.modalBody}>
              ...
              <Section title="Basic Details">
                <div style={A.formRow}>
                  <Field label="SKU *" hint="e.g. CG-SILK-001">
                    <input style={A.input} value={f.sku} required placeholder="CG-SILK-001" onChange={e=>set('sku',e.target.value)}/>
                  </Field>
                  <Field label="Saree Name *">
                    <input style={A.input} value={f.name} required placeholder="Kanjivaram Crimson Bridal" onChange={e=>set('name',e.target.value)}/>
                  </Field>
                </div>
                <Field label="Description">
                  <textarea style={{...A.input, minHeight:72, resize:'vertical'}} value={f.description}
                    placeholder="Describe the saree — fabric, occasion, weave…" onChange={e=>set('description',e.target.value)}/>
                </Field>
              </Section>
              ...
              <Section title="Pricing & Stock">
                <div style={A.formRow}>
                  <Field label="Selling Price (₹) *">
                    <input style={A.input} type="number" value={f.price} required min={0} placeholder="2499" onChange={e=>set('price',e.target.value)}/>
                  </Field>
                  <Field label="MRP (for strikethrough)" hint="optional">
                    <input style={A.input} type="number" value={f.originalPrice} min={0} placeholder="3000" onChange={e=>set('originalPrice',e.target.value)}/>
                  </Field>
                  <Field label="Stock Qty *">
                    <input style={A.input} type="number" value={f.stock} required min={0} placeholder="10" onChange={e=>set('stock',e.target.value)}/>
                  </Field>
                  <Field label="Sort Order" hint="lower = first">
                    <input style={A.input} type="number" value={f.sortOrder} onChange={e=>set('sortOrder',e.target.value)}/>
                  </Field>
                </div>
              </Section>
              ...
              <Section title="Visibility">
                <div style={A.toggleRow}>
                  {[['blouseIncluded','Blouse Included'],['isFeatured','Featured on Home'],['isActive','Active (visible in shop)']].map(([key,lbl])=>(
                    <label key={key} style={A.toggleLabel}>
                      <div style={f[key]?A.toggleOn:A.toggleOff} onClick={()=>set(key,!f[key])}>
              ...
              <Section title="Product Images (Primary Colour)">
                <div style={A.uploadHint}>First image = thumbnail. JPG, PNG or WebP. Compress under 300KB each.</div>
                <button type="button" style={A.uploadBtn} onClick={()=>imgRef.current.click()}>📸 Choose Images</button>
                <input ref={imgRef} type="file" accept="image/*" multiple style={{ display:'none' }}
                  onChange={e=>setImgFiles(Array.from(e.target.files))}/>
                {imgFiles.length>0 && <div style={A.fileChosen}>✓ {imgFiles.length} file(s) selected</div>}
                {editing?.imageUrls?.length>0 && (
                  <div style={{ marginTop:12 }}>
                    <div style={{ fontSize:11, color:'#6B5E4A', marginBottom:8 }}>Current images (click × to remove):</div>
                    <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
                      {editing.imageUrls.map((url,i)=>(
                        <div key={i} style={{ position:'relative' }}>
                          <img src={url} alt="" style={A.existThumb}/>
                          {i===0 && <div style={A.primaryBadge}>Primary</div>}
                          <button type="button" style={A.removeImgBtn} onClick={()=>removeImg(editing,url)}>×</button>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <Section title="Product Video (Primary Colour)">
                <div style={A.uploadHint}>Short MP4, under 30 sec. Max 100MB.</div>
                <button type="button" style={A.uploadBtn} onClick={()=>vidRef.current.click()}>🎬 Choose Video</button>
                <input ref={vidRef} type="file" accept="video/mp4,video/mov,video/webm" style={{ display:'none' }}
                  onChange={e=>setVidFile(e.target.files[0])}/>
                {vidFile && <div style={A.fileChosen}>✓ {vidFile.name} ({(vidFile.size/1024/1024).toFixed(1)}MB)</div>}
                {editing?.videoUrl && (
                  <div style={{ marginTop:8, fontSize:12, color:'#6B5E4A' }}>
                    Current: <a href={editing.videoUrl} target="_blank" rel="noreferrer" style={{ color:'#D4AB5A' }}>View ↗</a>
                  </div>
                )}
              </Section>

              <div style={A.modalFooter}>
                <button type="button" style={A.cancelBtn} onClick={()=>setShowForm(false)} disabled={saving}>Cancel</button>
      ...
      {/* ════ COLOUR VARIANTS MODAL ══════════════════════════ */}
      {showVariants && variantSaree && (
        <div style={A.modalBg} onClick={()=>setShowVariants(false)}>
          <div style={{ ...A.modal, maxWidth:720 }} onClick={e=>e.stopPropagation()}>

            <div style={A.modalHead}>
              <div>
                <div style={A.modalTitle}>Colour Variants</div>
                <div style={A.modalSub}>{variantSaree.name} · Primary: {variantSaree.color}</div>
              </div>
              <button style={A.closeBtn} onClick={()=>setShowVariants(false)}>✕</button>
            </div>

            <div style={{ ...A.modalBody, padding:'0 24px 24px' }}>

              {/* ── Existing variants ── */}
              {variantSaree.variants?.length > 0 && (
                <div style={{ marginTop:20, marginBottom:24 }}>
                  <div style={A.sectionHeading}>Existing Colour Variants</div>
                  <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                    {variantSaree.variants.map((v, i) => (
                      <div key={v._id} style={A.variantRow}>

                        {/* Thumbnail */}
                        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                          {v.imageUrls?.slice(0,3).map((url,j)=>(
                            <img key={j} src={url} alt="" style={{ width:48, height:60, objectFit:'cover', borderRadius:4, border:'1px solid #2A1E0A' }}/>
                          ))}
                          {!v.imageUrls?.length && (
                            <div style={{ width:48, height:60, background:'#1A1208', borderRadius:4, display:'flex', alignItems:'center', justifyContent:'center', fontSize:20, border:'1px solid #2A1E0A' }}>🥻</div>
                          )}
                        </div>

                        {/* Details */}
                        <div style={{ flex:1 }}>
                          <div style={{ fontWeight:600, color:'#F5F0E8', fontSize:14 }}>{v.color}</div>
                          <div style={{ fontSize:12, color:'#8A7560', marginTop:2 }}>
                            ₹{v.price?.toLocaleString('en-IN')} ·{' '}
                            <span style={v.stock===0?{color:'#F87171'}:v.stock<5?{color:'#F5C842'}:{color:'#4ADE80'}}>
                              {v.stock} in stock
                            </span>
                            {v.sku && <span style={{ color:'#5A4A30' }}> · {v.sku}</span>}
                          </div>
                        </div>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
                          ...
                          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                            <button
                              type="button"
                              style={{ ...A.uploadBtn, padding:'6px 12px', fontSize:11 }}
                              onClick={() => {
                                if (!variantImgRefs.current[v._id]) {
                                  variantImgRefs.current[v._id] = document.createElement('input');
                                  variantImgRefs.current[v._id].type = 'file';
                                  variantImgRefs.current[v._id].accept = 'image/*';
                                  variantImgRefs.current[v._id].multiple = true;
                                  variantImgRefs.current[v._id].onchange = (e) => {
                                    setVariantImgFiles(prev => ({
                                      ...prev,
                                      [v._id]: Array.from(e.target.files),
                                    }));
                                  };
                                }
                                variantImgRefs.current[v._id].click();
                              }}
                            >
                              📸 Photos
                            </button>
                            {variantImgFiles[v._id]?.length > 0 && (
                              <button
                                type="button"
                                style={{ ...A.saveBtn, padding:'6px 12px', fontSize:11 }}
                                onClick={() => uploadVariantImages(v._id)}
                              >
                                Upload {variantImgFiles[v._id].length}
                              </button>
                            )}
                          </div>

                          <button
                            style={{ ...A.btnTiny, color:'#F5C842', borderColor:'#5A4010' }}
                            onClick={() => updateVariantStock(v._id, v.stock)}
                          >
                            Stock
                          </button>

                          <button
                            style={{ ...A.btnTiny, color:'#F87171', borderColor:'#3D1A1A' }}
                            onClick={() => deleteVariant(v._id, v.color)}
                          >
                            Delete
                          </button>
                        </div>
                        ...
                        {variantImgFiles[v._id]?.length > 0 && (
                          <div style={{ width:'100%', fontSize:11, color:'#4ADE80', marginTop:4 }}>
                            ✓ {variantImgFiles[v._id].length} file(s) ready — click Upload
                          </div>
                        )}
                      </div>
                    ))}
              ...
              <div style={{ background:'#0F0B05', border:'1px solid #2A1E0A', borderRadius:10, padding:20 }}>
                <div style={A.sectionHeading}>Add a New Colour Variant</div>
                <p style={{ fontSize:12, color:'#5A4A30', marginBottom:16, lineHeight:1.6 }}>
                  Use this to add the same saree design in a different colour (e.g. Red, Blue, Green).
                  Each variant has its own price, stock and photos.
                </p>
                  ...
                  <div style={{ display:'flex', gap:12, flexWrap:'wrap', marginBottom:12 }}>
                    ...
                    <Field label="Price (₹) *">
                      <input style={A.input} type="number" value={variantForm.price} required min={0}
                        placeholder="2499" onChange={e=>vset('price',e.target.value)}/>
                    </Field>
                    <Field label="Stock *">
                      <input style={A.input} type="number" value={variantForm.stock} required min={0}
                        placeholder="5" onChange={e=>vset('stock',e.target.value)}/>
                    </Field>
                    <Field label="SKU" hint="optional">
                      <input style={A.input} value={variantForm.sku}
                        placeholder="CG-SILK-001-BLU" onChange={e=>vset('sku',e.target.value)}/>
                    </Field>
                  </div>

                  <div style={{ fontSize:12, color:'#6B5E4A', marginBottom:12 }}>
                    💡 After adding the variant, it will appear in the list above where you can upload its photos.
                  </div>

                  <div style={{ display:'flex', gap:10 }}>
                    <button type="submit" style={variantSaving?{...A.saveBtn,opacity:0.7}:A.saveBtn} disabled={variantSaving}>
                      {variantSaving ? 'Adding…' : '+ Add Colour Variant'}
                    </button>
                  </div>
                </form>
              </div>
...
function Section({ title, children }) {
  return (
    <div style={{ borderBottom:'1px solid #1A1208', paddingBottom:20, marginBottom:20, paddingTop:20 }}>
      <div style={{ fontSize:10, letterSpacing:'0.2em', textTransform:'uppercase', color:'#5A4A30', fontWeight:600, marginBottom:14 }}>
        {title}
      </div>
      {children}
    </div>
  ...
  return (
    <div style={A.loginWrap}>
      ...
      <div style={A.loginBox}>
        ...
        <div style={{ fontFamily:"'Playfair Display',serif", fontSize:28, color:'#F5E9CC', fontWeight:400, textAlign:'center', marginBottom:4 }}>City Girl</div>
        <div style={{ fontSize:10, letterSpacing:'0.3em', color:'#6B5E4A', textAlign:'center', marginBottom:36, textTransform:'uppercase' }}>Admin Panel</div>
        <form onSubmit={submit}>
          <input style={{...A.input,display:'block',width:'100%',marginBottom:12}} type="text" placeholder="Username" value={u} onChange={e=>setU(e.target.value)} autoFocus required/>
          <input style={{...A.input,display:'block',width:'100%',marginBottom:20}} type="password" placeholder="Password" value={p} onChange={e=>setP(e.target.value)} required/>
...
const A = {
  ...
  navBtn:      { display:'flex', alignItems:'center', gap:10, padding:'11px 12px', borderRadius:8, background:'none', border:'none', color:'#6B5E4A', cursor:'pointer', fontSize:13, fontFamily:"'Jost',sans-serif", fontWeight:500, width:'100%' },
  ...
  logoutBtn:   { margin:'0 12px', background:'none', border:'1px solid #2A1E0A', color:'#5A4A30', padding:'9px', borderRadius:6, cursor:'pointer', fontSize:12, fontFamily:"'Jost',sans-serif" },
  ...
  addBtn:      { background:'#B8892A', color:'#0D0A06', border:'none', borderRadius:6, padding:'10px 20px', fontSize:12, fontWeight:700, cursor:'pointer', letterSpacing:'0.08em', fontFamily:"'Jost',sans-serif", whiteSpace:'nowrap' },
  toast:       { margin:'16px 28px 0', padding:'12px 18px', borderRadius:8, border:'1px solid', fontSize:13, display:'flex', gap:10, alignItems:'center' },
  ...
  tr:          { borderBottom:'1px solid #1A1208' },
  ...
  btnEdit:     { background:'none', border:'1px solid #2A1E0A', borderRadius:4, padding:'5px 12px', fontSize:11, cursor:'pointer', color:'#C8B08A', fontFamily:"'Jost',sans-serif", marginRight:4 },
  ...
  modalHead:   { background:'#0F0B05', borderBottom:'1px solid #1E1408', padding:'20px 24px', display:'flex', justifyContent:'space-between', alignItems:'flex-start' },
  ...
  closeBtn:    { background:'none', border:'none', color:'#5A4A30', fontSize:22, cursor:'pointer', padding:0, lineHeight:1 },
  modalBody:   { padding:'0 24px 24px', maxHeight:'80vh', overflowY:'auto' },
  formRow:     { display:'flex', gap:14, flexWrap:'wrap', marginBottom:14 },
  input:       { padding:'10px 12px', background:'#0D0A06', border:'1px solid #2A1E0A', borderRadius:6, fontFamily:"'Jost',sans-serif", fontSize:13, color:'#C8B08A', outline:'none', width:'100%', boxSizing:'border-box' },
  toggleRow:   { display:'flex', gap:24, flexWrap:'wrap' },
  toggleLabel: { display:'flex', alignItems:'center', gap:10, cursor:'pointer' },
  toggleOn:    { width:38, height:20, background:'#B8892A', borderRadius:10, position:'relative', cursor:'pointer' },
  ...
  uploadBtn:   { background:'#1A1208', border:'1px solid #2A1E0A', color:'#C8B08A', borderRadius:6, padding:'9px 18px', fontSize:12, cursor:'pointer', fontFamily:"'Jost',sans-serif", fontWeight:500 },
  ...
  primaryBadge:{ position:'absolute', top:4, left:4, background:'rgba(184,137,42,0.9)', color:'#0D0A06', fontSize:9, padding:'2px 6px', borderRadius:3, fontWeight:700 },
  ...
  saveBtn:     { background:'#B8892A', color:'#0D0A06', border:'none', borderRadius:6, padding:'11px 28px', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:"'Jost',sans-serif", letterSpacing:'0.08em', display:'flex', alignItems:'center', gap:8 },
  sectionHeading: { fontSize:11, letterSpacing:'0.15em', textTransform:'uppercase', color:'#8A7560', fontWeight:600, marginBottom:14 },
  variantRow:  { background:'#0F0B05', border:'1px solid #2A1E0A', borderRadius:8, padding:'14px 16px', display:'flex', alignItems:'center', gap:14, flexWrap:'wrap' },
